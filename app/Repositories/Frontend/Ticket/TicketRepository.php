<?php
	...
	namespace App\Repositories\Frontend\Ticket;
	...
	*/
	class TicketRepository extends BaseRepository
	{
		...
		*/
		...
		public function __construct(TicketReply $replymodel, Purchasepackage $purchasepkgmodel)
		{
			$this->replymodel = $replymodel;
			$this->purchasepkgmodel = $purchasepkgmodel;
		...
		*/
		public function getForDataTable()
		{
            ...
            ->where(config('module.tickets.table').'.created_by', $user)->select([
		...
		public function getTotalNoOfQuery()
		{
			$createdby = access()->user()->id;
			return $this->query()->where(config('module.tickets.table').'.created_by', $createdby)->count();
		}
		public function getTotalNoOfQueryPurchase()
		{
			$createdby = access()->user()->id;
			return $this->purchasepkgmodel->query()->where(config('module.purchasepackage.table').'.created_by', $createdby)->sum('totalquery');
		}
		/**
		...
		*/
		public function create(array $input)
		{
			...
			if ($ticket) {
				...
				// DB::table('purchasepackage')->decrement('pendingquery');
				DB::table('purchasepackage')->where('created_by', access()->user()->id)->where('status', 1)->update(['pendingquery' => DB::raw('GREATEST(pendingquery - 1, 0)')]);
